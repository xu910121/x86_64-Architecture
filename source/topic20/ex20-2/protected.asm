...
;
;** 实验 20-2：打印 status信息及 stack
;

        finit                           ; 初始化 x87 FPU
        fldz                            ; 加载 0.0
        fld1                            ; 加载 1.0
        fld DWORD [memfp32]             ; 加载一个 NaN 数
        fld DWORD [memfp32 + 4]         ; 加载一个 + infinity（正无究大）数
        fld TWORD [memfp80]             ; 加载一个 denormal 数
        fxam                            ; 检查当前的 ST(0)状息
...
memfp32  dd 0x7fbfffff                  ; NaN 数编码
         dd 0x7f800000                  ; infinity 数编码
memfp80  dd 0xFFFFFFFF                  ; denormal 数编码
         dd 0x7FFFFFFF
         dw 0
